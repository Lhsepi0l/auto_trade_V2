...
### Recent Technical Outcomes
- On 2026-04-10, all active Discord surfaces were removed from the runtime and repository guidance. The old `v2/discord_bot/` package, Discord panel/views/commands/tests, Discord webhook notifier path, and stack/systemd Discord fallback options were deleted; the runtime is now web-operator-first only, with notification support centered on ntfy/webpush. Historical Discord mentions that remain elsewhere in this file are archival memory only, not active runtime guidance.
- On 2026-04-10, the control cleanup reached near-endgame structure. `v2/control/api.py` dropped to roughly 1.14k lines after moving runtime-config/bootstrap policy into `controller_runtime_config.py`, splitting position-management again into `controller_position_signal.py`, `controller_position_runner.py`, `controller_position_market.py`, and `controller_position_execution.py`, and centralizing readiness wrappers in `controller_readiness.py`. Internal control modules no longer import `v2.control.api` as an implicit utility bag; instead they share explicit helpers from `runtime_utils.py`, while `api.py` keeps only the backward-compatible shim surface (`FutureTimeoutError`, `time`, `_run_async_blocking`, etc.) for external tests/legacy touchpoints. Full `ruff` and full `pytest -q` were green after the refactor.
- On 2026-04-10, the remaining control-path cleanup was pushed one level deeper. `v2/control/controller_positions.py` was reduced to a tiny facade and its responsibilities were split into `controller_position_state.py` (persisted plan state I/O), `controller_position_management.py` (runner/weakness/time-stop policy), `controller_position_tpsl.py` (bracket repricing + placement), and `controller_position_close.py` (close/close_all flows). A new `v2/control/runtime_utils.py` now owns shared control helpers (`FutureTimeoutError`, float/bool/pct parsing, ISO time helpers, async bridge alias, logger), so internal control modules no longer import `v2.control.api` as an implicit utility surface; `cycle.py`, `recovery.py`, `profile_policy.py`, and `live_balance_helpers.py` were rewired to use explicit utility imports instead. `v2/control/controller_readiness.py` now centralizes readiness/status/balance wrapper glue, `v2/control/api.py` remains near 1.37k lines, and full `ruff` plus full `pytest -q` were green after preserving backward-compatible module-level shims on `api.py`.
- On 2026-04-10, the remaining `RuntimeController` hot paths around close/bracket/position-management orchestration and runtime risk-core calculation were split out of `v2/control/api.py` without changing strategy behavior. `v2/control/controller_positions.py` now owns persisted position-management state I/O, runner/weakness/time-stop management, bracket repricing + placement, and `close_position` / `close_all` flows; `v2/control/controller_risk_runtime.py` now owns universe/leverage/budget derivation, recent-block accounting, runtime equity/drawdown/cooldown refresh, and auto-risk trip flatten behavior. Backward-compatibility shims were preserved for module-level helpers that older control modules/tests monkeypatch (`FutureTimeoutError`, `time`, `_run_async_blocking`), and full `ruff` plus full `pytest -q` were green after the extraction.
- On 2026-04-10, `v2/control/api.py` was further de-spaghettified without changing strategy behavior by splitting controller lifecycle, status/report/read-model, and risk mutation/persistence glue into dedicated helper modules. `v2/control/controller_lifecycle.py` now owns start/stop/panic, worker-thread handling, user-stream callbacks, live-service boot/shutdown, reconcile, and scheduler tick paths; `v2/control/controller_status.py` now owns position read-model sourcing, periodic status emission, and daily report assembly; `v2/control/controller_risk.py` now owns runtime risk load/save plus operator-facing mutation endpoints (`get_risk`, `set_value`, `set_symbol_leverage`, `preset`, `clear_cooldown`). `RuntimeController` keeps thin wrappers over these helpers, `v2/control/api.py` dropped to roughly 2.4k lines, and full `ruff` plus full `pytest -q` were green after preserving the monkeypatchable `_loop_worker` start-path contract.
- On 2026-04-09, a live ops-gate latch bug was fixed: `close_position()` had been reusing `ops.flatten()` semantics that persist `paused=true/safe_mode=true`, which could leave the engine `RUNNING` while new entries stayed blocked as `ops_paused`. `v2/ops/control.py` now lets flatten callers opt out of latching ops mode, `v2/control/api.py` uses non-latching flatten for normal close paths, and `start()` now actively clears a latched ops gate even if the worker thread is already running. Regression coverage was added in `v2/tests/test_control_api.py`.
- On 2026-04-09, shared position-management infrastructure was introduced under `v2/management/` with a typed `PositionManagementSpec` contract and explicit lifecycle state machine (`ENTRY_ARMED -> RUNNER_ACTIVE -> EXIT_PENDING -> EXITED`). Live control now builds plans through `v2/control/position_management_runtime.py`, backtest metrics parse the same shared contract, and user-stream `ORDER_TRADE_UPDATE` trade events now have a fast path that can re-arm runner TP/SL immediately instead of waiting for the bracket poller. Full `ruff` and full `pytest -q` were green after this cleanup pass.
- On 2026-04-09, the active `ra_2026_alpha_v2_expansion_live_candidate` management contract was de-spaghettified around one explicit runner principle. `v2/strategies/ra_2026_alpha_v2.py` and `v2/config/config.yaml` now declare `management_policy=tp1_runner` plus explicit TP1/BE hints, while `v2/control/api.py` no longer injects hidden partial-take-profit defaults from `reward_risk_reference_r`. This aligns live control with backtest behavior and keeps TP-leg rearm behavior tied to an explicit runner plan instead of contradictory `single_exit_2r` naming.
- On 2026-04-09, live bracket polling was hardened so a TP leg fill no longer blindly tears down position management when the symbol still has remaining size. `v2/control/api.py` now re-checks the live position after TP confirmation; if the position is still open, it preserves `position_management_state`, re-arms TP/SL from the saved management plan, and avoids emitting a false full-close alert. Focused regression coverage was added in `v2/tests/test_control_api.py`.
- On 2026-04-06, Apple Web Push `BadJwtToken` failures were traced to the placeholder VAPID subject and resolved by normalizing the default subject away from `mailto:autotrader@local.invalid`. WebPushService now derives a usable HTTPS subject from the server's non-loopback IP when no explicit `WEBPUSH_SUBJECT` is configured (or when the placeholder subject is still present), which aligns better with the WireGuard-only private HTTPS deployment path.
- On 2026-04-06, Web Push VAPID signing was fixed by switching the pywebpush call path from a raw PEM string to a `py_vapid.Vapid01` instance created via `from_pem(...)`. This resolved hosted send failures after successful device subscription where pywebpush/py-vapid rejected the private key with ASN.1 parse errors even though the stored PKCS8 PEM itself was valid.
- On 2026-04-06, operator PWA cache busting was hardened by switching `/operator` asset versioning from file mtime to content hash, ensuring iPhone home-screen apps fetch updated JS/CSS after deploys instead of silently reusing stale cached assets. Push subscribe button clicks are now also logged immediately as `push_subscribe_click` so future onboarding failures can be distinguished from dead-button/cache issues.
- On 2026-04-06, operator PWA troubleshooting was upgraded with client-side telemetry. The web console now pushes client diagnostics and JS failures back into operator events, renders push state before the rest of the dashboard, and preserves diagnostic text when the current device is not subscribed, so iPhone onboarding failures can be identified from server logs instead of looking like a silent no-op.
- On 2026-04-06, the operator PWA push onboarding UX was hardened so unsupported client states no longer look like a dead button. The push connect flow now computes explicit client-side diagnostics (secure context, standalone mode, Notification/ServiceWorker/PushManager support, permission state), exposes them in the operator push card, and surfaces the exact reason via feedback when the operator taps `푸시 연결` instead of silently leaving the button inert.
- On 2026-04-06, the web operator gained a first-party iPhone-oriented PWA/Web Push path so mobile alerts no longer have to depend on Discord or hosted `ntfy`. The `/operator` surface now serves a manifest + service worker, exposes device subscription/test routes, and shows install/permission/subscription state in the console UI; runtime notification plumbing gained a `webpush` provider with DB-backed VAPID key generation and subscription storage via `RuntimeStorage`, while `WEBPUSH_ENABLED=true` now promotes Web Push to the active notifier path once dependencies are installed and devices subscribe. Focused coverage was added in `v2/tests/test_webpush_service.py`, `v2/tests/test_operator_web_routes.py`, `v2/tests/test_operator_service.py`, and `v2/tests/test_v2_env_and_notify.py`.
- On 2026-04-05, ntfy mobile push delivery was stabilized against hosted `ntfy.sh` daily publish quotas. `v2/notify/runtime_events.py` now suppresses scheduler-origin normal wait-state pushes (`no_candidate`, general `blocked/risk_rejected`) for `ntfy` while keeping manual-tick attention, critical failures, and deduped `position_open` heartbeat behavior; `v2/notify/notifier.py` now records HTTP response bodies in `last_error` so `429 daily message quota reached` is visible from operator status/report surfaces; focused regressions were updated in `v2/tests/test_control_api.py` and `v2/tests/test_v2_env_and_notify.py`.
- On 2026-04-04, runtime observability for the new long rebound lane was upgraded so operators can tell whether `alpha_drift` is merely armed or actually confirmed. `v2/control/cycle.py` now emits explicit non-notifying operator/log events for `alpha_drift_setup_queued` and `alpha_drift_confirmed`, with setup timestamp propagation from `v2/strategies/ra_2026_alpha_v2.py`; operator-event text coverage and control-path regressions were added in `v2/tests/test_operator_service.py` and `v2/tests/test_control_api.py`.
- On 2026-04-04, `ra_2026_alpha_v2_expansion_verified_q070` and `ra_2026_alpha_v2_expansion_live_candidate` were moved from pure `alpha_expansion` into a dual-lane posture (`alpha_expansion` + `alpha_drift`) so the runtime can keep short continuation entries while also taking long rebound/delayed-continuation entries. `v2/strategies/ra_2026_alpha_v2.py` now lets `alpha_drift` confirm under strict shared cost settings when the original setup edge was already strong (`edge_shortfall` near-pass on stored setup edge), and both active profiles now pin the long drift lane to `LONG only`, `expiry=8`, `tp=1.8R`, `time_stop=16`, alongside the short overextension guard (`expansion_short_break_distance_atr_max=1.3`). Focused coverage was added/updated in `v2/tests/test_ra_2026_alpha_v2.py` and `v2/tests/test_v2_config_loader.py`.
- On 2026-04-04, `ra_2026_alpha_v2_expansion_live_candidate` was hardened against late chase shorts after waterfall drops. `v2/strategies/ra_2026_alpha_v2.py` now supports `expansion_short_break_distance_atr_max` and rejects `alpha_expansion` short entries with `short_overextension_risk` when the close is too far below the breakout band in ATR terms; the live-candidate profile now pins this guard at `1.3`, and focused regressions cover both a still-valid moderate short expansion and an overextended short-expansion rejection in `v2/tests/test_ra_2026_alpha_v2.py` plus profile-load coverage in `v2/tests/test_v2_config_loader.py`.
- Fixed live private-stream uncertainty recovery on 2026-04-01: `v2/control/api.py` now clears `state_uncertain` on `private_ok` only when the uncertainty was caused by the latest transient user-stream disconnect/error reason and the stream is no longer stale, preventing prolonged false `상태 불확실` after short user-stream reconnects while preserving non-stream uncertainty causes; focused regressions were added in `v2/tests/test_control_api.py`.
...
- Fixed a live TP/SL disappearance bug on 2026-03-28: `v2/control/api.py` bracket poller no longer treats a temporarily missing leg as an immediate TP/SL fill while the position is still open; it now requires flat-position or recent-fill confirmation before cleanup/alerts and otherwise repairs brackets from the persisted position-management plan.
- Hardened bracket placement atomicity on 2026-03-28: `v2/tpsl/brackets.py` now cancels any already-accepted leg and marks runtime state `CLEANED` if the second algo leg fails during live placement, preventing one-sided/orphaned bracket state; focused regressions were added in `v2/tests/test_control_api.py` and `v2/tests/test_tpsl_brackets.py`.
- Finalized web-first operator alignment on 2026-03-20: `/operator` now covers report + concise operator guidance for normal daily use, `run_stack.sh` / `install_systemd_stack.sh` / systemd/docs default to web-first startup, and Discord is retained only as optional fallback / emergency-use-only.
- Added first-step ntfy mobile push integration on 2026-03-20: shared `v2/notify` abstraction now supports hosted `ntfy.sh` publish flow plus future self-hosted base URL swap, runtime/control notifications are routed independently of Discord, periodic status spam is suppressed for ntfy, and key operator events (start/stop, manual-tick attention results, stale/recovery, auto-risk, TP/SL, forced close, report delivery) now emit concise phone-friendly alerts.
- Added alert-quality hardening on 2026-03-21: shared notifier now supports lightweight per-event suppression via dedupe keys + time windows, important one-shot alerts remain unsuppressed, standalone boot notification now uses the structured path, and status/operator read models expose last notification delivery state (provider, status, event type/title, timestamps, suppression count) for operator verification.
- Fixed production operator-control edge cases on 2026-03-21: `readyz` no longer falls to false solely because the startup reconcile timestamp ages out while live user-stream sync remains healthy, scheduler-driven web logs/mobile alerts now follow `notify_interval_sec` together, scheduler interval changes refresh default stale/watchdog thresholds, and the web console now labels engine-loop cadence vs. alert cadence more explicitly to reduce operator misconfiguration.
- Finalized branch-first production rollout on 2026-03-21: source-only commits were pushed to `migration/web-operator-panel` (`7dbf6d9`, `ca8c1f1`, `e584d85`, `8708554`, `8b7872f`) while keeping `main` untouched as rollback target, with local/generated noise (`.env`, `.venv`, `__pycache__`, `.vscode`) intentionally excluded from commits.
- Confirmed practical remote-access posture on 2026-03-21: LAN access requires the control API to bind to the Pi LAN IP (`192.168.50.29:8101` in the validated setup), LTE access works through the existing WireGuard path when UFW explicitly allows the WireGuard client subnet (`10.6.0.0/24`) to reach port `8101`, and systemd should keep `--no-discord-bot` in `ExecStart` for web-first operation.
- Added standalone `operator_ui/` frontend workspace on 2026-03-20 using React + Vite + Tailwind CSS v4 + `shadcn/ui` official Vite initialization, with import alias wiring, base `Button` component generation, and successful production build verification.
...
- Web-first operator surface was extended with server-persisted operator events (`operator_events` SQLite table), `/operator/api/events`, `/operator/api/logs`, a dedicated `/operator/logs` page, and a shared top navigation/header so dashboard event feed survives refreshes and longer operational history can be reviewed from web without relying on browser-local state.
- `/operator/logs` now supports `offset`-based pagination with UI `이전/다음` controls, so operators can move past the latest 1000-row window instead of being limited to a single fetched batch.
