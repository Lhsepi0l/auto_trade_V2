...
from v2.discord_bot.views.panel import MarginBudgetModal, PanelView
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_non_admin_click_denied_and_no_api_call(monkeypatch: pytest.MonkeyPatch) -> None:
    api = SimpleNamespace(
        set_config=AsyncMock(),
        get_status=AsyncMock(return_value={"engine_state": {"state": "RUNNING"}}),
    )
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_invalid_input_shows_error_and_no_api_call(monkeypatch: pytest.MonkeyPatch) -> None:
    api = SimpleNamespace(
        set_config=AsyncMock(),
        get_status=AsyncMock(return_value={"engine_state": {"state": "RUNNING"}}),
    )
