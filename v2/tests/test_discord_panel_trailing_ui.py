...
from v2.discord_bot.ui_labels import TRAILING_BUTTON_LABEL
from v2.discord_bot.views.panel import ADMIN_ONLY_MSG, AdvancedPanelView, TrailingConfigModal
...
async def test_trailing_modal_submit_calls_set_config(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    await _find_button(view, TRAILING_BUTTON_LABEL).callback(it_click)  # type: ignore[arg-type]
...
async def test_trailing_permission_gate_denies_non_admin(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    it = _FakeInteraction()
    await _find_button(view, TRAILING_BUTTON_LABEL).callback(it)  # type: ignore[arg-type]
