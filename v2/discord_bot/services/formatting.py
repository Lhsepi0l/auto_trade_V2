...
def _coerce_float(v: JSONValue, *, default: float = 0.0) -> float:
    try:
        return float(str(v))
...
def _coerce_int(v: JSONValue, *, default: int = 0) -> int:
    try:
        return int(str(v))
    except (TypeError, ValueError):
        return default
...
def _fmt_money(x: JSONValue, *, digits: int = 4) -> str:
    try:
        return f"{float(str(x)):.{digits}f}"
    except (TypeError, ValueError):
        return str(x)


def _fmt_int(x: JSONValue) -> str:
    try:
...
def _fmt_time(ts: JSONValue) -> str:
    if not ts:
        return "-"
...
def _safe_float(v: JSONValue) -> float | None:
    try:
        return float(str(v))
    except (TypeError, ValueError):
        return None
...
def _as_dict(v: object) -> JSONPayload:
    return v if isinstance(v, dict) else {}


def _parse_ts(ts: JSONValue) -> datetime | None:
    if ts is None:
        return None
    if isinstance(ts, datetime):
...
def _next_tick_eta(payload: JSONPayload) -> str:
    sched = _as_dict(payload.get("scheduler"))
    tick_sec = _safe_float(sched.get("tick_sec")) or 1800.0
...
def _regime_to_kor(raw_regime: JSONValue) -> tuple[str, str]:
...
def _candidate_reject_stage_to_kor(raw_stage: JSONValue) -> str:
...
def _reason_to_kor(raw_reason: JSONValue) -> str:
...
def _format_top_counts(counts: JSONPayload, *, limit: int = 6) -> str:
    if not counts:
        return "-"
    try:
        items = list(counts.items())
        sorted_items = sorted(
            [(k, _coerce_int(v)) for k, v in items if _coerce_int(v) > 0],
            key=lambda x: x[1],
            reverse=True,
        )
...
def _translate_rejection_map(reasons: JSONPayload) -> dict[str, str]:
...
def _format_scan_stats(stats: JSONPayload) -> str:
    if not stats:
        return "-"
    ...
    if usable_tfs is not None:
        if isinstance(usable_tfs, (list, tuple)):
            parts.append(f"TF {len(usable_tfs)}개({', '.join([str(tf) for tf in usable_tfs])})")
        else:
...
def _format_candidate_scores(scores: JSONPayload) -> str:
    if not scores:
        return "-"
    items: list[str] = []
    for tf in _SCORING_TIMEFRAME_ORDER:
...
def _position_side(amount: JSONValue, side_hint: JSONValue = None) -> str:
...
def _first_position(positions: JSONValue) -> tuple[str, str, float, str]:
    ...
    for sym, row in positions.items():
        ...
        if fallback_symbol == "-":
            ...
            fallback_unrealized = _coerce_float(row.get("unrealized_pnl"), default=0.0)

        amt = _safe_float(row.get("position_amt"))
        if amt is None:
            continue
        if abs(amt) > 1e-12:
...
def _collect_unrealized(positions: JSONValue, pnl: JSONPayload) -> float:
    total = 0.0
    if isinstance(positions, dict):
        for row in positions.values():
            if isinstance(row, dict):
                total += _coerce_float(row.get("unrealized_pnl"), default=0.0)
    if total != 0:
        return total

    if isinstance(pnl, dict):
        return _coerce_float(pnl.get("last_unrealized_pnl_usdt"), default=0.0)
    return 0.0


def format_status_payload(payload: JSONPayload) -> str:
    ...
    raw_enabled_symbols = binance.get("enabled_symbols")
    if isinstance(raw_enabled_symbols, (list, tuple)):
        enabled_symbols = [str(x) for x in raw_enabled_symbols if str(x).strip()]
    ...
    daily_pnl = pnl.get("daily_pnl_pct")
    ...
    if isinstance(raw_scoring_drift_details, (list, tuple)):
        scoring_drift_details = list(raw_scoring_drift_details)
    ...
    if scoring_setup_signature:
        ...
        setup_parts = []
        if setup_tfs:
            if isinstance(setup_tfs, (list, tuple)):
                setup_parts.append(f"TF={','.join([str(x) for x in setup_tfs])}")
            else:
                setup_parts.append(f"TF={setup_tfs}")
    ...
    rejected_text = _format_top_counts({k: v for k, v in (rejected or {}).items()})
    if rejected_text != "-":
...
def format_report_payload(payload: JSONPayload) -> str:
    if not isinstance(payload, dict):
