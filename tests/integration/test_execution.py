...
@pytest.mark.integration
def test_entry_blocked_when_request_exceeds_budget_cap() -> None:
    ex = FakeBinanceRest()
    ex.available = 1000.0
    svc = _svc(ex, _cfg(capital_mode="FIXED_USDT", capital_usdt=10.0, margin_use_pct=1.0, max_leverage=1))
    out = svc.enter_position({"symbol": "BTCUSDT", "direction": "LONG", "exec_hint": "LIMIT", "qty": 1.0})
    assert out.get("blocked") is True
    assert str(out.get("block_reason")) == "ENTRY_EXCEEDS_BUDGET_CAP"


@pytest.mark.integration
def test_entry_uses_live_sizing_when_qty_and_notional_missing() -> None:
    ex = FakeBinanceRest()
